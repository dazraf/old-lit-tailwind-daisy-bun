...
type StringOrStringFunction = string | ((params: { [key: string]: string | undefined }) => string);
...
export class Breadcrumbs extends AppStyledElement() {
  ...
  private routeMap: Map<string, PathRouteConfigWithBreadcrumb> = new Map<string, PathRouteConfigWithBreadcrumb>();
  ...
  private getBreadcrumbs(routes: PathRouteConfigWithBreadcrumb[], link: string = "") {
    const result =
      routes
        .map((r) => {
          const pathRouteConfig = r as PathRouteConfigWithBreadcrumb;
          const urlPattern = Breadcrumbs.getUrlPatternOrCreate(pathRouteConfig.path);
